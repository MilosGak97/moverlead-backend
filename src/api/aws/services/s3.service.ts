...
export class S3Service {
    ...
    async readBrightdataSnapshot(snapshot_id: string) {
        const params = {Bucket: process.env.AWS_S3_BUCKET_NAME_BRIGHTDATA, Key: snapshot_id};
        ...
        try {
            ...
            // Log raw string size BEFORE parsing
            const sizeInBytes = Buffer.byteLength(data, 'utf8');
            const sizeInKB = sizeInBytes / 1024;
            console.log(`📦 Raw snapshot string size: ${sizeInBytes} bytes (${sizeInKB.toFixed(2)} KB)`);

            // Parse and return
            return JSON.parse(data);

        } catch (error) {
            console.error(`❌ Error reading JSON from S3 (${snapshot_id}):`, error);
            throw new Error("Could not read JSON file from S3");
        }
    }
}
